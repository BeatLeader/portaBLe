...
namespace portaBLe
{
    public class InferPublish
    {
        ...
        private MapProcessing dataProcessing = new MapProcessing();

        private static object inferenceSessionLock = new();
        private static InferenceSession inferenceSessionAcc = new InferenceSession(Path.Combine("wwwroot", "model_sleep_4LSTM_acc.onnx"), new Microsoft.ML.OnnxRuntime.SessionOptions { IntraOpNumThreads = NumThreads, ExecutionMode = ExecutionMode.ORT_SEQUENTIAL });
        private static InferenceSession inferenceSessionSpeed = new InferenceSession(Path.Combine("wwwroot", "model_sleep_4LSTM_speed.onnx"), new Microsoft.ML.OnnxRuntime.SessionOptions { IntraOpNumThreads = NumThreads, ExecutionMode = ExecutionMode.ORT_SEQUENTIAL });
        private static InferenceSession tagSession = new InferenceSession(Path.Combine("wwwroot", "tagging_model.onnx"));
        ...
        public (List<float>, List<double>, int) PredictHitsForMap(DifficultySet difficulty, double bpm, double njs, double timescale = 1)
        {
            ...
            for (int i = 0; i < predictionsArraysAcc.Count; i++)
            {
                ...
                for (int j = 0; j < batchPred.Length; j++)
                {
                    ...
                    var inp = batchInp.Skip(MapProcessing.preSegmentSize).Take(batchInp.Count - MapProcessing.preSegmentSize - MapProcessing.preSegmentSize).ToArray()[0];
        ...
        private double GetAccForMultiplierScale(double multiplier)
        {
            ...
            throw new InvalidOperationException("The multiplier value did not match any curve point.");
        }
